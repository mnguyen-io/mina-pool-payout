...
export async function signTransactionsToSend(payoutsToSign: PayoutTransaction[], keys: keypair, outputDirectory: string) {
    ...
    payoutsToSign.forEach((payout: PayoutTransaction) => {
        ...
        try {
            ...
            fs.writeFileSync(`${outputDirectory}/${nonce}.json`, data);
            nonce++;
        }
